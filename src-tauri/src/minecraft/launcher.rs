...
use super::{java::Version, modpack::install_modpack, mojang_meta::get_minecraft};
...
pub(crate) async fn launch(config: Config) -> Result<()> {
    ...
    get_minecraft(&target_version, &data_dir, &c).await?; // done
    ...
    // install_modpack(&slug, &version, &data_dir, &c).await?; // no

    launch_minecraft(&java, &data_dir, "net.minecraft.client.main.Main", &target_version).await?; // no

    Ok(())
}

async fn launch_minecraft(java: &PathBuf, instance: &DataDir, main_class: &str, version: &str) -> Result<()> {
    ...
    let output = Command::new(java.display().to_string())
        .args([
            &format!("-Djava.library.path={}libraries", instance.path.display().to_string()),
            "-cp",
            &instance
                .get_library_dir("com/mojang/minecraft/1.19.2/client.jar")?
                .display()
                .to_string(),
            main_class,
            "-Xmx2048M",
            ...
            version,
            "--assetsDir",
            &instance.get_assets_dir().display().to_string(),
        ])
        .output()?;
