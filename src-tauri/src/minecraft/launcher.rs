...
pub(crate) async fn launch(config: Config) -> Result<()> {
    ...
    let mut class_path = get_minecraft(&target_version, &data_dir, &c).await?; // done
    ...
    let mod_path = install_modpack(&slug, &target_version, &data_dir, &c, &mut class_path).await?; // no
...
async fn launch_minecraft(
    ...
    dbg!(class_path.contains("fabric-loader"));
    dbg!(&class_path);

    // Create a temporary file.
    let temp_directory = env::temp_dir();
    let temp_file = temp_directory.join("debug-class_path.txt");
    ...
    writeln!(&mut file, "{}", class_path).unwrap();
