...
pub(crate) async fn set_instance_config(
    ...
    let value = toml::Value::try_from(config).context("Serializing config to toml value failed")?;
    write_to_file(
        &toml::to_string(&value).context("String conversion of toml config failed")?,
        &pack_toml,
    )?;
    Ok(())
}
